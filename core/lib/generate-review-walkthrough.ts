...
const identityForUnit = (
  unit: ReviewUnit,
): Exclude<NarrativeIdentity, 'review-focus' | 'single'> =>
  unit.kind === 'commit'
...
const promptOptionsForUnit = (
  ...
  const fromLabel = request.comparison.before.head?.label.text ?? 'Unavailable';
  const toLabel = request.comparison.after.head?.label.text ?? 'Unavailable';
  return {
    ...base,
...
const aggregatePromptOptions = (
  ...
  const fromLabel = generationRequest.review.comparison.before.head?.label.text ?? 'Unavailable';
  const toLabel = generationRequest.review.comparison.after.head?.label.text ?? 'Unavailable';
  return {
    ...base,
    versionCompareRange: {
      fromLabel,
...
const buildPiStructuredRetryPrompt = (prompt: string, error: unknown) => `${prompt}

Your previous response could not be accepted because it failed structural validation:
${error instanceof Error ? error.message : String(error)}

Retry once. Return only the requested JSON object. It must include a top-level \`chapters\` array and every other field required by the supplied schema.`;
...
const runNarrativeTask = (
  ...
  return {
    ...
    run: async ({ profile: requestedProfile, semanticInput: requestedInput, signal }) => {
      ...
      const normalizeResult = (result: Awaited<ReturnType<ReviewWalkthroughRunModel>>) => ({
      ...
      try {
        ...
        if (requestedProfile.agent !== 'pi') {
          throw error;
        }
        signal?.throwIfAborted();
        const retry = await runModel({
          profile: requestedProfile,
          prompt: buildPiStructuredRetryPrompt(prompt, error),
          semanticInput: requestedInput,
          signal,
          state,
        });
        return normalizeResult(retry);
      }
    },
    semanticInput,
  };
...
export async function generateReviewWalkthrough(
  ...
  if (focusResult.status !== 'ready') {
    return {
      ...
      reusableComponents: [...taskResult.components, ...focusResult.components],
      status: 'failed',
    };
  }
