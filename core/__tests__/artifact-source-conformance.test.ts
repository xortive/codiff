...
import { createCommitArtifactRequestKey, type CommitArtifactRequest } from '@nkzw/codiff-core';
...
import { type CommitArtifact, type ReviewArtifactProject } from '../lib/review-artifacts.ts';
...
const commitArtifactRequest: CommitArtifactRequest = {
  commitSha: commitSha as unknown as CommitArtifactRequest['commitSha'],
  parentSha: parentSha as unknown as CommitArtifactRequest['parentSha'],
};
...
const gitLabArtifact = async (): Promise<CommitArtifact> => {
  ...
  const artifact = (
    await source.readCommitArtifacts([commitArtifactRequest], new AbortController().signal)
  ).get(createCommitArtifactRequestKey(commitArtifactRequest));
  if (!artifact) {
