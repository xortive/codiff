...
const profileFor = (scope: TreeInspectionScope): GenerationProfile => ({
  agent: 'codex',
  authoringVersion: 'walkthrough-v5-narrative-2',
...
test('retries one structurally invalid Pi narrative response', async () => {
  const piProfile = (scope: TreeInspectionScope): GenerationProfile => ({
    ...profileFor(scope),
    agent: 'pi',
  });
  let attempt = 0;
  const runModel = vi.fn<ReviewWalkthroughRunModel>(async ({ profile }) => {
    attempt += 1;
    return {
      generationMetadata: metadataFor(profile),
      response:
        attempt === 1 ? { focus: 'Missing chapters.', kind: 'narrative', title: 'Invalid' } : draft,
    };
  });

  const result = await generateReviewWalkthrough({
    narrativeProfile: piProfile,
    runModel,
    selection: {
      range,
      relation: 'target-comparison',
  ...
  expect(runModel.mock.calls[1]?.[0].prompt).toContain('top-level `chapters` array');
});
