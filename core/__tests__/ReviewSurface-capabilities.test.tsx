...
test('shows the file filter only in Tree and keeps History unfiltered', async () => {
  const entries = [
    {
      author: 'Ada Lovelace',
  ...
  expect(view.container.querySelector<HTMLInputElement>('.sidebar-search')?.placeholder).toBe(
    'Filter files',
  );
  await act(async () => findButton(view.container, 'Walkthrough')?.click());
  expect(view.container.querySelector('.sidebar-search')).toBeNull();
  await act(async () => findButton(view.container, 'History')?.click());
  expect(view.container.querySelector('.sidebar-search')).toBeNull();
