...
test('normalizes GitLab range artifact patches into renderer-parseable unified diffs', () => {
  ...
  expect(range.files[0]?.lineCount).toEqual({ additions: 1, deletions: 1 });
  expect(parsePatchFiles(range.files[0]?.patch ?? '')).toMatchObject([
    { files: [{ additionLines: ['new\n'], deletionLines: ['old\n'], name: 'src/app.ts' }] },
  ]);
});

test('keeps collapsed GitLab files patchless without inventing counts', () => {
  const range = createGitLabRangeArtifact({
  ...
  expect(range.files[0]).toMatchObject({ coverage: 'truncated', path: 'src/large.ts' });
  expect(range.files[0]?.lineCount).toBeUndefined();
  expect(range.files[0]?.patch).toBeUndefined();
});
