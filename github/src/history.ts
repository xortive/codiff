...
const asLineCount = (additions: unknown, deletions: unknown) => {
  const added = asNumber(additions);
  const deleted = asNumber(deletions);
  return added != null &&
...
const countUnifiedPatchLines = (patch: string) => {
  let additions = 0;
  let deletions = 0;
...
const normalizeGitHubArtifactFile = (value: unknown): ArtifactFile | null => {
  ...
  const lineCount =
    asLineCount(value.additions, value.deletions) ??
    (patchBody.trim() ? countUnifiedPatchLines(patchBody) : null);
...
export const createGitHubArtifactSource = ({
  ...
  return {
    ...
    async readStackAndRange({ headSha: head, requestedBaseSha: base }, signal) {
      ...
      const effectiveBase =
        isRecord(first.merge_base_commit) && typeof first.merge_base_commit.sha === 'string'
          ? (gitSha(first.merge_base_commit.sha) ?? base)
          : base;
...
export const compareGitHubReviewVersionAggregate = async ({
  ...
  const replay = await computeVersionComparePreferringReplay({
    from: {
      ...
      headSha: fromHead,
      label: versionOptionLabelText(from),
    ...
    to: {
      ...
      headSha: toHead,
      label: versionOptionLabelText(to),
