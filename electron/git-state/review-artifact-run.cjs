...
const createPersistentReviewArtifactSource = (source, options) => {
  ...
  const readCachedStackAndRange = async (baseSha, headSha, signal) => {
    ...
    return cached &&
      ...
      cached.range.baseSha === baseSha &&
      cached.range.headSha === headSha &&
      cached.stack.baseSha === baseSha &&
  ...
  return {
    ...
    async readCommitArtifacts(commits, signal) {
      ...
      const misses = requested.filter((commit) => !artifacts.has(commit.commitSha));
      if (misses.length === 0) {
        return artifacts;
      }
      ...
      for (const commit of misses) {
        const artifact = acquired.get(commit.commitSha);
        if (
          !artifact ||
          artifact.commitSha !== commit.commitSha ||
          artifact.parentSha !== commit.parentSha
        ...
        artifacts.set(commit.commitSha, artifact);
        if (
          isCompleteArtifact(artifact) &&
          hasArtifactProvenance(artifact, options.provenanceKind)
        ) {
    ...
    async readStackAndRange(baseSha, headSha, signal) {
      ...
      const acquired = await source.readStackAndRange(baseSha, headSha, signal);
      signal.throwIfAborted();
      if (
...
const createFallbackReviewArtifactSource = (primary, fallback) => ({
  ...
  async readCommitArtifacts(commits, signal) {
    ...
    const preferredRequests = requested.filter(
      (commit) => !isCompleteArtifact(cachedFallback.get(commit.commitSha)),
    );
    let preferred = new Map();
    ...
    const fallbackRequests = preferredRequests.filter(
      (commit) => !isCompleteArtifact(preferred.get(commit.commitSha)),
    );
    let replacements = new Map();
    ...
    for (const commit of preferredRequests) {
      const preferredArtifact = preferred.get(commit.commitSha);
      const replacement = replacements.get(commit.commitSha);
      if (isCompleteArtifact(replacement) || !preferredArtifact) {
        if (replacement) artifacts.set(commit.commitSha, replacement);
      } else {
        artifacts.set(commit.commitSha, preferredArtifact);
      }
    }
    return artifacts;
  },
  async readStackAndRange(baseSha, headSha, signal) {
    ...
    let preferred;
    try {
      preferred = await primary.readStackAndRange(baseSha, headSha, signal);
    ...
    try {
      const replacement = await fallback.readStackAndRange(baseSha, headSha, signal);
...
const listArtifactRepositoryHistory = async (
  ...
  const [{ stack }, baseHistory] = await Promise.all([
    run.readStackAndRange(baseSha, headSha, control.signal || run.signal),
  ...
  const reviewEntries = stack.commits.map((commit) => ({
    ...
    ...(artifactDiffStat(artifacts.get(commit.sha))
      ? { diffStat: artifactDiffStat(artifacts.get(commit.sha)) }
      : {}),
  ...
  if (baseEntries[0]) {
    const diffStat = artifactDiffStat(artifacts.get(baseEntries[0].sha));
    if (diffStat) {
...
const materializeReviewUnitFromArtifacts = async (repoRoot, project, unit) => {
  ...
  if ((unit.kind === 'revised' || unit.kind === 'ambiguous') && unit.before && unit.after) {
    ...
    try {
      ...
      const before = artifacts.get(unit.before.sha);
      const after = artifacts.get(unit.after.sha);
      if (!before || !after) {
        recordUnitReplayMetric(run, project, unit, {
          outcome: 'unavailable',
  ...
  try {
    ...
    const artifact = artifacts.get(selection.commit.sha);
