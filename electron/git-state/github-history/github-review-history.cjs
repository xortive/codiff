  ...
  const readRangeArtifact = (base, head) =>
    readImmutableHistory(
      ...
      async () => {
        ...
        const { range } = await run.readStackAndRange(base, head, control.signal || run.signal);
        return range;
      },
  ...
  return {
    ...
    readCommitStack: (base, head) =>
      readImmutableHistory(
        ...
        async () => {
          const run = await getArtifactRun(repoRoot, source, control);
          return (await run.readStackAndRange(base, head, control.signal || run.signal)).stack
            .commits;
        },
        { shareInFlight: control.signal == null },
      ),
