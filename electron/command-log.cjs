...
const measureLoggerOverhead = () => {
  const startedAt = performance.now();
  for (let index = 0; index < overheadSampleCount; index += 1) {
    JSON.stringify({
      args: sanitizeArgs([
        'api',
        '--header',
        'Authorization: Bearer benchmark-secret',
        '/projects/example?private_token=benchmark-secret&ref=main',
      ]),
      command: 'glab',
      details: sanitizeDetails({ action: 'diagnostic-overhead', requestId: `sample-${index}` }),
      event: 'start',
      id: index,
      kind: 'command',
  ...
  return {
    averageRecordMs: Math.round((totalMs / overheadSampleCount) * 10_000) / 10_000,
    sampleCount: overheadSampleCount,
    totalMs,
  };
};
...
const configureCommandLog = (logsDirectory, options = {}) => {
  ...
  commandLogPath = path;
  nextTimingId = 0;
  const loggerOverhead = measureLoggerOverhead();
  enqueueWrite(async () => {
