...
const { readPullRequestSectionContent, readPullRequestState } =
  require('../git-state/pull-request.cjs') as {
    ...
    readPullRequestSectionContent: (
      ...
      path: string,
    ) => Promise<import('../../core/types.ts').DiffSection>;
  };
...
const { createGitLabPosition, readMergeRequestSectionContent, readMergeRequestState } =
...
test('returns a GitHub Range Artifact before exact file hydration', async () => {
  ...
  const { baseSha, headSha } = await createReviewRange(directory, {
...
`,
  ...
  try {
    ...
    const hydrated = await readPullRequestSectionContent(directory, state.source, 'src/app.ts');
...
`,
  ...
  try {
    ...
    const hydrated = await readMergeRequestSectionContent(directory, state.source, 'src/app.ts');
    expect(hydrated).toMatchObject({
      loadState: 'ready',
      newFile: { contents: 'new src/app.ts\n', name: 'src/app.ts' },
      oldFile: { contents: 'old src/app.ts\n', name: 'src/app.ts' },
